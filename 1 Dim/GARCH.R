...
sim.GARCH = function(eta, rho){
  Y = rep(0, n + ncut)
  sigma = rep(1, n + ncut)
  Y[1] = 1 + sigma[1] * eta[1]
  for (t in 2:(n + ncut)){
    sigma[t] = sqrt(2 + rho[1] * sigma[t-1]^2 * eta[t-1]^2 +
                      rho[2] * sigma[t-1]^2)
    Y[t] = 1 + sigma[t] * eta[t]
  }
...
test = function(err.id){
  # simulate GARCH(1,1) data with different error dist.
  ...
  # garch.fit = ugarchfit(spec=garch.spec, data=Y)
  # X = garch.fit@fit[["residuals"]]
  ...
  logL = function(theta){
    ...
    sigma2 = rep(1, n)
    # t = 1
    sum = - log(sigma2[1]) - 1 / 2 * (Y[1] - mu)^2 / sigma2[1]
    for (t in 2:n){
      sigma2[t] = omega + rho1 * (Y[t-1] - mu)^2 + rho2 * sigma2[t-1]
      sum = sum - log(sigma2[t]) - 1 / 2 * (Y[t] - mu)^2 / sigma2[t]
    }
    return(-sum)
  }
  optim = constrOptim(theta = c(1,1,0.1,0.7),
                      f = logL,
                      grad = NULL,
                      ui = matrix(c(0, 0, -1, -1,
                                    0, 1, 0, 0,
  ...
  fit = function(Y, theta){
    ...
    eps = Y - mu
    for (t in 2:n){
      sigma[t] = sqrt(omega + rho1 * eps[t-1]^2 +
                        rho2 * sigma[t-1]^2)
    }
    res = eps / sigma
    out = list(residuals = res, sigma = sigma)
    return(out)
  }
  garch.fit = fit(Y, theta_hat)
  X = garch.fit$residuals
  sigma = garch.fit$sigma
  ...
  for (b in 1:nB){
    ...
    logL_b = function(theta){
      ...
      sum = 0
      for (t in 1:n){
        sum = sum - log(sigma[t]^2) - 1 / 2 * (Y_b[t] - mu)^2 / sigma[t]^2
      }
      return(-sum)
    }
    optim_b = constrOptim(theta = c(1,1,0.1,0.7),
                          f = logL_b,
                          grad = NULL,
                          ui = matrix(c(0, 0, -1, -1,
                                        0, 1, 0, 0,
                                      ...
                                      nrow = 4,ncol = 4,byrow = T),
                          ci = c(-1, 0.1, 0, 0))
    theta_hat_b = optim_b$par
    fit_b = function(Y, theta){
      mu = theta[1]
      omega = theta[2]
      rho1 = theta[3]
      rho2 = theta[4]
    ...
    garch.fit_b = fit_b(Y_b, theta_hat_b)
    X.b = garch.fit_b$residuals
